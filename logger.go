...
func LoggerInit(level string, logFormat string) {
	...
	if strings.ToLower(logFormat) == "json" {
		slog.SetDefault(slog.New(NewTracingHandler(slog.NewJSONHandler(os.Stderr, &opts))))
	} else {
		slog.SetDefault(slog.New(NewTracingHandler(slog.NewTextHandler(os.Stderr, &opts))))
	}
}
